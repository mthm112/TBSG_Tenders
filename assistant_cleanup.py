...
def is_business_hours():
    ...
    Protects: 6:55am UTC daily sync + Carol's working hours (starts 7:15am UK)
    ...
    # Protected hours: 6:30am-5pm UTC weekdays
    # Protects:
    # - 6:55am UTC: Daily FTP sync (runs Mon-Fri, takes ~11 minutes)
    # - Carol's working hours starting 7:15am UK time
    #   GMT (winter): 7:15am UK = 7:15 UTC (sync finishes ~7:06am UK)
    #   BST (summer): 7:15am UK = 6:15 UTC (sync finishes ~8:06am UK)
    # Safe range: 6:30-17:00 UTC covers sync + working hours
    is_business_hour = (now.hour == 6 and now.minute >= 30) or (7 <= now.hour < 17)
    
    is_business = is_weekday and is_business_hour
    
    if is_business:
        if now.hour == 6:
            reason = f"Daily sync time: {now.strftime('%A %H:%M UTC')} (FTP sync runs 6:55am UTC Mon-Fri)"
        else:
            reason = f"Business hours detected: {now.strftime('%A %H:%M UTC')} (Carol's hours: 7:15am start UK time)"
    else:
        if not is_weekday:
